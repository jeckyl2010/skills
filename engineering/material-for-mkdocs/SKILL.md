...
- Duplicate H1 in page body: MkDocs renders the `title` from frontmatter as the page heading. A `# Title` line in the document body produces a double heading. Do not include a top-level H1 in the body — start directly with `--8<-- "includes/legend-glossary.md"` then `## Introduction`. All five values pages checked in session had this issue.
